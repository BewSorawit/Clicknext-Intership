...
def register(user: UserCreate, db: Session = Depends(get_db)):
    user_in_db = db.query(User).filter(
        User.email == user.email or User.user_name == user.user_name).first()
    if user_in_db:
        raise HTTPException(
    ...
    new_user = User(user_name=user.user_name,
                    email=user.email, password=hashed_password, api_quota_limit=user.api_quota_limit
                    )
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
...
def login(user: UserLogin, db: Session = Depends(get_db)):
    ...
    access_token = create_access_token(
        data={"user_name": user_in_db.user_name}, expires_delta=access_token_expires)
    refresh_token = create_refresh_token(
        data={"user_name": user_in_db.user_name}, expires_delta=refresh_token_expires)
